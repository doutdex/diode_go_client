...
func TestMethodSignature(t *testing.T) {
	...
	idexp := crypto.Sha3Hash([]byte(exp))[:4]
...
func TestMultiPack(t *testing.T) {
	...
	sig := crypto.Sha3Hash([]byte("bar(uint32,uint16)"))[:4]
...
func TestInputFixedArrayAndVariableInputLength(t *testing.T) {
	...
	fixedarrin1value1 = common.LeftPadBytes(fixedarrin1[0].Bytes(), 32)
	fixedarrin1value2 = common.LeftPadBytes(fixedarrin1[1].Bytes(), 32)
	dynarroffsetBigInt := big.NewInt(int64(256 + ((len(strin)/32)+1)*32))
	dynarroffset = util.PaddingBytesPrefix(dynarroffsetBigInt.Bytes(), 0, 32)
	dynarrlength = make([]byte, 32)
	dynarrlength[31] = byte(len(dynarrin))
	dynarrinvalue1 = common.LeftPadBytes(dynarrin[0].Bytes(), 32)
	dynarrinvalue2 = common.LeftPadBytes(dynarrin[1].Bytes(), 32)
...
func TestABI_MethodById(t *testing.T) {
	...
	if _, err = abi.MethodById(crypto.Sha3Hash([]byte{})); err == nil {
